...
func (c *Client) writePump() {
	...
	for {
		...
		case message, ok := <-c.send:
			log.Println("Message to " + c.id)
			log.Println("Read Msg", string(message))
...
func serveWs(hub *Hub, w http.ResponseWriter, r *http.Request) {
	...
	client := &Client{hub: hub, conn: conn, send: make(chan []byte, 256), id: connectionID}
